...
Assert-Contains $defaults 'proxmox_backup_storage_prune_keep_last:' "Canonical retention variable is required."
Assert-Contains $defaults 'proxmox_backup_prune_keep_last\s*\|\s*default\(2\)' "Legacy retention input must remain compatible."
Assert-Contains $defaults 'proxmox_backup_is_mountpoint\s*\|\s*default\(true\)' "Legacy mountpoint input must remain compatible."
...
Assert-Contains $tasks 'proxmox_backup_storage_candidate\.is_mountpoint\s*==\s*proxmox_backup_storage_mount_path' "Readback must accept Proxmox's canonical mount path representation."
